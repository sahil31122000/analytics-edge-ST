...
#Read the dataset----
data_adult<-read.csv("R Codes/annual_income.csv")
...
abline(h=quantile(data_adult$hours.per.week))  
abline(h=quantile(data_adult$hours.per.week, .99))
...
recast_data <- data_adult_rescale %>%
  mutate(education = 
           factor(ifelse(education == "Preschool" | education == "10th" | 
